...
def get_conn():
    """
    Returns a global connection instance cached across context processes.
    Enables Write-Ahead Logging (WAL) mode for resilient background multi-threading.
    """
...
def init_db():
    """ Initializes tables and executes column migrations cleanly. """
    with conn:
        cursor = conn.cursor()
        ...
        # Seed initial system configuration identity
...
def compress_img(image_file):
    """ Scales and squashes reference files to minimize DB storage footprint. """
    ...
    try:
        ...
        st.error(f"Image processing breakdown: {e}")
        return None
...
def multimodal_input(key):
    ...
    with c2:
        st.caption("🖼️ Upload Reference Documentation")
    ...
    if canvas is not None and canvas.image_data is not None:
        ...
        if np.any(arr[:, :, 3] > 0):  # Check if canvas contains active brush strokes
            try:
                ...
                st.error(f"Sketch compilation fault: {e}")
...
# =========================================================
# MODULE 1: INTERACTIVE ANALYTICS DASHBOARD
# =========================================================
    ...
    if courses.empty:
        ...
        for _, row in courses.iterrows():
            ...
            with st.expander(f"⚙️ {row['course_name']} [{row['category']}] — {status_tag}"):
                ...
                # Dynamic Filtered Note Retrieval
                ...
                for _, note in notes_df.iterrows():
                    with st.container(border=True):
                        ...
                        n1, n2 = st.columns(2)
                        if note['sketch_data']:
                            n1.image(base64.b64decode(note['sketch_data']), caption="Vector Sketch Blueprint File")
                        if note['image_blob']:
                            n2.image(note['image_blob'], caption="Reference Spec Capture Attachment")
...
# =========================================================
    ...
    for _, j in journal_df.iterrows():
        with st.container(border=True):
            ...
            if j['sketch_data']:
                jc1.image(base64.b64decode(j['sketch_data']), caption="Diagnostic Workspace Sketch Render")
            if j['image_blob']:
                jc2.image(j['image_blob'], caption="Hardware Component Photo Reference")
...
# =========================================================
    ...
    # Extract identity configuration vector from database array
...
# =========================================================
    ...
    mit_syllabus_structure = [
        {"Topic": "Mathematics", "Code": "18.06", "Curriculum focus": "Linear Algebra, Vector Transformations, Spectral Decomposition"},
        {"Topic": "Computer Science", "Code": "6.0001", "Curriculum focus": "Algorithmic Complexity, Structural Python Optimization, Object Frameworks"},
        {"Topic": "Systems Engineering", "Code": "RES.6-007", "Curriculum focus": "Fourier Analysis, Continuous Signal Filters, Convolution Operations"},
        {"Topic": "Control Theory", "Code": "6.302", "Curriculum focus": "PID Tuning Metrics, Root Locus Stability Analysis, State-Space Models"},
        {"Topic": "Hardware & Robotics", "Code": "2.12", "Curriculum focus": "Kinematic Transforms, Spatial Jacobians, Actuator Dynamics Profiling"}
    ]
    ...
    with b2:
        ...
        if restore_file is not None:
            if st.button("🚨 Overwrite System Core Matrix"):
                try:
                    # Dissolve connection handles before modifying raw files
                    conn.close()
                    with open(DB_NAME, "wb") as f:
                        f.write(restore_file.getbuffer())
                    # Re-initialize context resource binding
